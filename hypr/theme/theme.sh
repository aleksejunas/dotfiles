...
# shellcheck disable=SC2154  # Variables are defined in sourced theme files
...
source_default() {
    ...
    # shellcheck disable=SC2034  # altforeground may be used by sourced files
...
source_pywal() {
    ...
    check_wallpaper() {
        if [[ -d "$WALLDIR" ]]; then
            WFILES="$(find "$WALLDIR" -maxdepth 1 -type f | wc -l)"
            if [[ "$WFILES" == 0 ]]; then
...
apply_rofi() {
    # rofi : colors
    cat > "${PATH_ROFI}/shared/colors.rasi" <<-_EOF_
	* {
	    background:     ${background};
...
apply_waybar() {
    # waybar : colors
    cat > "${PATH_WAYB}/colors.css" <<-_EOF_
	/** ********** Colors ********** **/
...
apply_wlogout() {
    # wlogout : colors
    cat > "${PATH_WLOG}/colors.css" <<-_EOF_
	/** ********** Colors ********** **/
	@define-color background      ${background};
	@define-color background-alt1 ${modbackground[1]};
	@define-color background-alt2 ${modbackground[2]};
